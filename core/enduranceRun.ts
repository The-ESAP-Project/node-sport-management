...
// 长跑标准数据（时间以秒为单位存储）
const enduranceRunStandard: TestStandard = {
  // 男生长跑标准（分:秒）
  male: {
    // 高一、高二、高三的长跑范围和对应分数
    grade1: [
 ...
 * @returns 分数和等级信息
 */
export function calculateEnduranceRunScore(
  seconds: number,
  gender: Gender = "male",
  grade: Grade = 1
  ...
  // 获取对应性别和年级的标准
  const gradeKey = `grade${grade}` as keyof typeof enduranceRunStandard.male;
...
export default calculateEnduranceRunScore;
